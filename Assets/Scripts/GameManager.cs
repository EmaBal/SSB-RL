...
public class GameManager : MonoBehaviour {
	...
	public void AddPoints(int amount)
	{
		...
		// +1 live every 10 points
		if(score%10 == 0)
		{
			lives++;
			refreshGUI();
			// set animation for new live icon
			UIExtraLives[lives-2].GetComponent<Animator> ().SetTrigger("AddLive");
			_audio.PlayOneShot(AddLiveSFX);
